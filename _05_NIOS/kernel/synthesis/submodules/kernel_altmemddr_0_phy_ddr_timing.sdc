...
################################################################################
# Generated by:12.1
# variation name : kernel_altmemddr_0_phy
# family : Cyclone IV E
# speed_grade : 6
# local_if_drate : Full
# pll_ref_clk_mhz : 100.0
# mem_if_clk_mhz : 166.7
# mem_if_preset : Custom (Micron MT47H64M16)
...
source "[list [file join [file dirname [info script]] kernel_altmemddr_0_phy_ddr_timing.tcl]]"
...
################################################################################
set pin_file_name "kernel_altmemddr_0_phy_ddr_pins.tcl"
...
# instantiations.
################################################################################
proc add_requirements_for_instance {corename instance_name t_name board_name ISI_name kernel_altmemddr_0_phy_use_flexible_timing} {
	upvar 1 $t_name t
...
# Create the PLL input clock and derive clocks on the PLL outputs
	...
	if {$ck_pll_clock_id == -1} {
		foreach {msg_type msg} $msg_list {
			post_message -type $msg_type "kernel_altmemddr_0_phy_ddr_timing.sdc: $msg"
		}
		post_message -type warning "kernel_altmemddr_0_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(ck_p)]"
	} else {
		...
		if {$pll_ref_clk_id != -1} {
			...
			post_message -type info "kernel_altmemddr_0_phy_ddr_timing.sdc: Could not find PLL clocks for $ck_pll_clock. Creating PLL base clocks"
			# Attempt to recover
...
# Find the TimeQuest name for the resync clock. If it is not found, create one.
	...
	if {$resync_pll_ref_clk_id != -1} {
		...
		post_message -type warning "kernel_altmemddr_0_phy_ddr_timing.sdc: Failed to find PLL input clock pin driving $resync_clock"
	}
...
################################################################################
	...
	if {$dqs_pll_clock_id == -1 || $dq_pll_clock_id == -1} {
		...
		post_message -type warning "kernel_altmemddr_0_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join [get_all_dqs_pins $pins(dqsgroup)]]"
	} else {
...
################################################################################
	foreach dqsgroup $pins(dqsgroup) {
		...
		if {$kernel_altmemddr_0_phy_use_flexible_timing} {
			# DQS output clock
			...
			if {$kernel_altmemddr_0_phy_use_flexible_timing && ($::quartus(nameofexecutable) ne "quartus_fit")} {
				lappend ::dqs_clocks $dqs_out_clockname
			}
		}
		# endif $kernel_altmemddr_0_phy_use_flexible_timing
		if {$kernel_altmemddr_0_phy_use_flexible_timing} {
...
set_false_path -to [get_pins -compatibility_mode ${instname}_alt_mem_phy_inst|dpio|dqs_group\[*\].dq\[*\].dqi|auto_generated|input_*_*\[0\]|clrn]
	...
	if {($::quartus(nameofexecutable) == "quartus_fit") || !$kernel_altmemddr_0_phy_use_flexible_timing} {
...
################################################################################
	...
	if {$ac_pll_clock_id == -1} {
		...
		post_message -type warning "kernel_altmemddr_0_phy_ddr_timing.sdc: Failed to find PLL clock for pins [join $pins(addrcmd)]"
	} else {
...
################################################################################
	...
	if {$kernel_altmemddr_0_phy_use_flexible_timing && (($::quartus(nameofexecutable) ne "quartus_fit") && ($::quartus(nameofexecutable) ne "quartus_map"))} {
...
foreach inst $instance_list {
	...
	add_requirements_for_instance $corename $inst t board ISI $kernel_altmemddr_0_phy_use_flexible_timing
